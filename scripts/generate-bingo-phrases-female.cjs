 ...
 * Voz: cgSgspJ2msm6clMCkdW9 - Jessica (Playful, Bright, Warm) - FEMENINA
 * Genera N_call1.mp3 y N_call2.mp3 para números 1-75
 * Destino: public/audio/numbers-female/
 * Total: 150 archivos
...
const OUTPUT_DIR = path.join(__dirname, '..', 'public', 'audio', 'numbers-female');
...
async function main() {
  ...
  console.log('GENERADOR DE FRASES DE BINGO - VOZ FEMENINA (Jessica)');
  console.log('Números: 1–75  |  2 frases cada uno  |  150 archivos');
  console.log('Destino: public/audio/numbers-female/');
  console.log('='.repeat(65));
  ...
  console.log('Necesario total:   ' + totalChars + ' caracteres');

  if (initialQuota === 0) {
    console.log('\nERROR: Sin cuota disponible. Esperá la renovación mensual.');
    process.exit(1);
  }
  ...
  // Agrupar tasks por número para procesar call1+call2 juntos o ninguno
  const porNumero = [];
  for (let i = 1; i <= 75; i++) {
    const t1 = tasks.find(t => t.num === i && t.slot === 'call1');
    const t2 = tasks.find(t => t.num === i && t.slot === 'call2');
    porNumero.push({ num: i, pair: [t1, t2], chars: t1.text.length + t2.text.length });
  }

  let success = 0;
  let errors = 0;
  let charsUsed = 0;
  let cuotaRestante = initialQuota;
  const failed = [];
  const pendientes = [];

  console.log('\n' + '='.repeat(65));
  console.log('Generando archivos (call1 + call2 por número)...');
  console.log('='.repeat(65) + '\n');

  for (const { num, pair, chars } of porNumero) {
    // Si no alcanza la cuota para el par completo, marcar como pendiente
    if (chars > cuotaRestante) {
      pendientes.push(num);
      console.log(num + '. [OMITIDO]   Cuota insuficiente (' + chars + ' chars necesarios, ' + cuotaRestante + ' disponibles)');
      continue;
    }
    ...
    for (const task of pair) {
      ...
      try {
        ...
        cuotaRestante -= task.text.length;
        success++;
  ...
  console.log('='.repeat(65));
  console.log('Voz:                   Jessica (cgSgspJ2msm6clMCkdW9)');
  ...
  if (pendientes.length > 0) {
    console.log('\nNÚMEROS PENDIENTES (' + pendientes.length + '): ' + pendientes.join(', '));
    console.log('→ Volvé a correr el script cuando se renueve la cuota.');
  }
  console.log('='.repeat(65) + '\n');

  if (errors > 0) {
    console.log('ADVERTENCIA: ' + errors + ' archivos fallaron. Volvé a ejecutar el script para reintentar.');
  } else if (pendientes.length === 0) {
    console.log('Todos los audios generados exitosamente.');
  } else {
    console.log('Sesión completada. Quedan ' + pendientes.length + ' números para la próxima sesión.');
  }
}
